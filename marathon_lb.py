...
def send_admin_commands(apps):
    logger.info("sending admin commands")
    for app in apps:
        backend = app.appId[1:].replace('/', '_') + '_' + \
            str(app.servicePort)
        for backendServer in app.backends:
            if backendServer.draining:
                serverName = re.sub(
                    r'[^a-zA-Z0-9\-]', '_',
                    backendServer.host + '_' + str(backendServer.port))
                try:
                    client = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
                    client.connect("/var/run/haproxy/socket")
                    client.sendall("set server {}/{} state drain\n"
                                   .format(backend, serverName).encode())
                except OSError as e:
                    logger.exception(e)
                except IOError as e:
                    logger.exception(e)
...
class MarathonEventProcessor(object):
    ...
    def reset_from_tasks(self):
        ...
        regenerate_config(self.__apps,
                          ...
                          self.__ssl_certs,
                          self.__templater)
        send_admin_commands(self.__apps)
