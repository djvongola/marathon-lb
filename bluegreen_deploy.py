    ...
    for app in apps['apps']:
        if 'labels' in app and \
            ...
            if prev_colour == 'blue':
                ...
                colour = 'blue'
    return (colour, next_port, existing_app)


def get_hostports_from_backends(hmap, backends):
    hostports = {}
...
def find_tasks_to_kill(tasks, hostports):
    tasks_to_kill = set()
    for task in tasks:
        if task['host'] in hostports and \
                hostports[task['host']] in task['ports']:
            tasks_to_kill.add(task['id'])
    return list(tasks_to_kill)


def check_if_tasks_drained(args, app, existing_app, target_instances):
    ...
    url = args.marathon_lb + "/haproxy?stats;csv"
    ...
    for addr in addrs:
        try:
            ...
            logger.exception("Caught exception when retrieving HAProxy"
                             " stats from " + nexturl.geturl())
            # HAProxy was probably reloading when we hit it, try again later
            addrs.append(addr)
            time.sleep(0.1)
    ...
    logger.info("Found {} app backends".format(len(backends)))
    ...
    # Check if current draining backends are finished serving requests
    draining_backends = \
        [b for b in backends if b[hmap['status']] == 'DRAIN']
    ...
    hostports = get_hostports_from_backends(hmap, draining_backends)

    tasks_to_kill = find_tasks_to_kill(existing_app['tasks'], hostports)

    logger.info("There are {} drained backends, "
                "about kill & scale for these tasks:\n{}"
                .format(len(tasks_to_kill), "\n".join(tasks_to_kill)))

    if app['instances'] == target_instances:
...
def start_deployment(args, app, existing_app):
    url = args.marathon + "/v2/apps"
    data = json.dumps(app)
    headers = {'Content-Type': 'application/json'}
...
def process_json(args, out=sys.stdout):
    ...
    if args.force or query_yes_no("Continue with deployment?"):
        start_deployment(args, app, existing_app)
